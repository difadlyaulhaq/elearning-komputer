...
import { ChevronLeft, Loader, ChevronDown, FolderOpen } from 'lucide-react';
...
export default function CategoryForm({ id }: CategoryFormProps) {
  ...
  useEffect(() => {
    if (id) {
      const fetchCategory = async () => {
        try {
          // Fetch categories and find the specific one to avoid backend changes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading(id ? 'Menyimpan perubahan...' : 'Menambahkan kategori...');
    ...
    try {
      ...
      if (response.ok) {
        toast.success(id ? 'Kategori berhasil diperbarui' : 'Kategori berhasil ditambahkan');
        router.push('/admin/categories');
        router.refresh();
      } else {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-sm text-gray-500 font-medium">Memuat data kategori...</span>
        </div>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1.5">Nama Kategori *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="relative">
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">Ikon Kategori</label>
                  <button
                    type="button"
              ...
              <div className="flex gap-3 pt-6 border-t border-gray-100">
                ...
                >
                  ...
                  <span>{id ? 'Simpan Perubahan' : 'Simpan Kategori'}</span>
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4 sticky top-28">
            ...
            <div className="border border-gray-150 rounded-xl p-4 bg-gray-50 flex flex-col items-center">
              <div className="w-full bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4 hover:shadow-md transition-shadow">
                ...
                <div>
                  <h4 className="font-bold text-gray-900 text-base line-clamp-1">{formData.name || 'Nama Kategori'}</h4>
                  <p className="text-xs text-gray-500 mt-1 line-clamp-2 min-h-[2rem]">
                    {formData.description || 'Deskripsi singkat mengenai isi kategori ini...'}
                  </p>
                </div>
            ...
            <p className="text-[10px] text-gray-400 text-center leading-relaxed">
              Tampilan di atas merepresentasikan bagaimana kategori ini akan ditampilkan pada beranda catalog kursus pegawai.
            </p>
          </div>
        </div>
      </div>
