...
export function LessonPlayerDesktop({
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#0F0F0F] min-h-screen">
      {/* Desktop Header */}
      <header className="bg-[#141414] border-b border-white/5 px-6 py-3.5 sticky top-0 z-10 flex items-center justify-between">
        <div className="flex items-center gap-4 flex-1 min-w-0">
          <Link
            href={`/learning/course/${courseId}`}
            className="flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors flex-shrink-0 group"
          >
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-4">
          {lesson.duration && (
            <div className="hidden md:flex items-center gap-1.5 text-xs text-gray-400 bg-white/5 px-3 py-1.5 rounded-full">
              <Clock size={12} />
      ...
      <div className="flex-1 flex flex-col lg:flex-row">
        ...
        <div className="flex-1 flex flex-col">
          ...
          {/* Info + Content Area */}
          <div className="flex-1 bg-[#0F0F0F] p-4 md:p-6 max-w-5xl mx-auto w-full">
            {/* Text content */}
            {lesson.contentType === "text" && (
              <div className="bg-[#141414] rounded-2xl border border-white/5 p-6 mb-4">
                <div className="flex items-center gap-3 mb-5 pb-4 border-b border-white/5">
                  <div className="w-11 h-11 bg-[#C5A059]/10 rounded-xl flex items-center justify-center border border-[#C5A059]/20">
                    <BookOpen size={20} className="text-[#C5A059]" />
                  </div>
                  <div>
                    <h2 className="text-base font-bold text-white">Artikel Pembelajaran</h2>
                    <p className="text-xs text-gray-400">{lesson.duration || '10'} menit membaca</p>
                  </div>
                </div>
            ...
            {lesson.contentType === "file-upload" && (
              <div className="bg-[#141414] rounded-2xl border border-white/5 p-8 text-center mb-4">
                <div className="w-16 h-16 bg-[#C5A059]/10 border border-[#C5A059]/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <Download size={28} className="text-[#C5A059]" />
                </div>
                <h2 className="text-lg font-bold text-white mb-1">File Materi</h2>
                <p className="text-sm text-gray-400 mb-5">Silakan unduh atau buka file materi melalui tombol di bawah.</p>
                <a
                  href={lesson.url}
                  target="_blank"
            ...
            {isVideoContent && (
              <div className="bg-[#141414] rounded-2xl border border-white/5 p-5 mb-4">
            ...
            {lesson.attachmentUrl && lesson.attachmentName && (
              <div className="bg-[#141414] rounded-2xl border border-white/5 p-5 mb-4">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-3 p-3.5 bg-white/5 hover:bg-[#C5A059]/10 hover:border-[#C5A059]/20 rounded-xl border border-white/5 transition-all group"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-white truncate">{lesson.attachmentName}</p>
                    <p className="text-xs text-gray-400 mt-0.5">Klik untuk mengunduh</p>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  {prevLesson ? (
                    <Link
                      ...
                      className="flex items-center gap-2 px-4 py-2.5 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl transition-all text-sm group"
                    >
                      <ChevronLeft size={15} className="text-gray-400" />
                      <div>
                        <p className="text-[10px] text-gray-500 uppercase tracking-wider">Sebelumnya</p>
                        <p className="text-sm font-semibold text-white line-clamp-1 max-w-[150px]">{prevLesson.title}</p>
                      </div>
                    </Link>
                  ) : (
                    <div />
                  ...
                  {nextLesson && (
                    ...
                    >
                      <div className="text-right">
                        <p className="text-[10px] text-gray-500 uppercase tracking-wider">Selanjutnya</p>
                        <p className="text-sm font-semibold text-white line-clamp-1 max-w-[150px]">{nextLesson.title}</p>
                      </div>
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 px-6 py-3 font-bold text-sm rounded-xl transition-all duration-300 ${
                    isVideoCompleted && !isUpdating
                      ? "bg-gradient-to-r from-[#C5A059] to-[#D4AF6A] text-black shadow-lg shadow-[#C5A059]/20 hover:shadow-[#C5A059]/30 hover:scale-[1.02]"
